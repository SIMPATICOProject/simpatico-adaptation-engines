...
var waeUI2 = (function () {
  ...
  function Singleton() {
	...
	this.init = function(config) {
		...
		if (config.endpoint) {
			waeEngine2.init({endpoint: config.endpoint});
		}
	 ...
	 */
	this.loadModel = function(moduleUri, idProfile) {
		var moduleUri = $("[data-simpatico-workflow]").attr('data-simpatico-workflow');
		if (!!idProfile) {
			this.idProfile = idProfile;
		}
		showLoader();
		waeEngine2.loadModel(moduleUri, this.idProfile).then(
			function(result) {
				moduleLoaded(result);
			},
			function(error) {
    ...
    this.enable = function(idProfile) {
    	if (waeEngine2.isLoaded()) {
    		for(var key in blockMap) {
    			if(blockMap.hasOwnProperty(key)) {
    				showElement(key, "HIDE");
    			}
    		}
    		waeEngine2.restartBlock();
				waeEngine2.nextBlock().then(
					function(result) {
						doActions(result);
						if (waeEngine2.getActualBlockId()) logger().logBlockStart(simpaticoEservice, waeEngine2.getActualBlockId());
					},
					function(error) {
	...
	this.reset = function(stay){
		...
		resetBlock(waeEngine2.getActualBlockId());
	...
	function moduleLoaded(map) {
		...
		waeEngine2.nextBlock().then(
			function(result) {
				doActions(result);
				logger().logBlockStart(simpaticoEservice, waeEngine2.getActualBlockId());
			},
			function(error) {
				moduleErrorMsg(error);
			}
	...
	function showElement(simpaticoId, state) {
		var element = waeEngine2.getSimpaticoBlockElement(simpaticoId);
		if(element !== null) {
			if(state == "SHOW") {
	...
	function resetBlock(simpaticoId) {
		var element = waeEngine2.getSimpaticoBlockElement(simpaticoId);
		if(element != null) {
			var container = waeEngine2.getSimpaticoContainer();
			if(container != null) {
	...
	function editBlock(simpaticoId) {
		...
		if(element != null) {
			...
			if(container != null) {				
				//add prev button
				if(waeEngine2.getActualBlockIndex() > 0) {
					$(containerInt).append(createPrevButton());
				}
				//add next button
				if(waeEngine2.getActualBlockIndex() < (waeEngine2.getBlocksNum() - 1)) {
				...
				var descriptions = waeEngine2.getBlockDescription();
				var message = "";
				if(descriptions) {
				...
				setTimeout(function() { waeEngine2.initBlock(); }, 200);
			}
		}
	};
	...
	function nextBlock() {
		if (waeEngine2.getActualBlockId()) logger().logBlockEnd(simpaticoEservice, waeEngine2.getActualBlockId());
		showLoader();
		waeEngine2.nextBlock().then(
			function(result) {
				doActions(result);
